...
public class Legend : MonoBehaviour {
    ...
    private void ConstructGradient(JSONNode legendSpecs, ref ChannelEncoding channelEncoding)
    {
        ...
        gameObject.GetComponent<Microsoft.MixedReality.Toolkit.Utilities.GridObjectCollection>().Rows = 3;             // TODO: Update this if no ticks are shown.
        gameObject.GetComponent<Microsoft.MixedReality.Toolkit.Utilities.GridObjectCollection>().CellHeight = 0.08f;
        gameObject.GetComponent<Microsoft.MixedReality.Toolkit.Utilities.GridObjectCollection>().UpdateCollection();
    }
    ...
    private void ConstructSymbols(JSONNode legendSpecs, ref ChannelEncoding channelEncoding, GameObject markPrefab)
    {
        ...
        if (channelEncoding.channel == "color")
        {
            foreach(string domainValue in channelEncoding.scale.domain)
            {
                ...
                legendValueInstance.GetComponent<Microsoft.MixedReality.Toolkit.Utilities.GridObjectCollection>().UpdateCollection();
            }

            gameObject.GetComponent<Microsoft.MixedReality.Toolkit.Utilities.GridObjectCollection>().Rows = channelEncoding.scale.domain.Count + 1;
            gameObject.GetComponent<Microsoft.MixedReality.Toolkit.Utilities.GridObjectCollection>().Rows = channelEncoding.scale.domain.Count + 1;
            gameObject.GetComponent<Microsoft.MixedReality.Toolkit.Utilities.GridObjectCollection>().CellHeight = 0.05f;   // TODO: Set to height of each legendValue.
            gameObject.GetComponent<Microsoft.MixedReality.Toolkit.Utilities.GridObjectCollection>().UpdateCollection();
