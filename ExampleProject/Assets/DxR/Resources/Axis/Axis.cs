...
namespace DxR
{
    public class Axis : MonoBehaviour
    {
        ...
        private void EnableThresholdFilter(JSONNode axisSpecs, DxR.Scale scale)
        {
            ...
            // APO: abilita il filtro threshold sull'asse
            ...
            DxR.SliderThreshold sliderControl = slider.GetComponent<DxR.SliderThreshold>();
            float domainMin = float.Parse(scale.domain[0]);
            float domainMax = float.Parse(scale.domain[1]);

            sliderControl.UpdatePosition();
            sliderControl.Init(domainMin, domainMax);

            slider.gameObject.name = dataField;

            interactionsObject.EnableAxisThresholdFilter(dataField);
            sliderControl.sliderControl.OnValueUpdated.AddListener(interactionsObject.ThresholdFilterUpdated);
