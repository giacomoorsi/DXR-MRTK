...
namespace DxR
{
    ...
    public class MarkText : Mark
    {
        ...
        private void SetFollowing(string value)
        {
            if(value == "True")
            {
                gameObject.GetComponent<Microsoft.MixedReality.Toolkit.UI.Billboard>().enabled = true;
            } else
            {
                gameObject.GetComponent<Microsoft.MixedReality.Toolkit.UI.Billboard>().enabled = false;
            }
        }
